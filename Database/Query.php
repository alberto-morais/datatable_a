...
class Query {
    ...
    */
	 ...
	 *  @return self
	 */
	public function join ( $table, $condition, $type='', $protectIdentifiers = true )
	{
	...
	public function orderField ( $order )
	{
		if ( $order === null ) {
			return $this;
		}
	...
	protected function _build_field( $addAlias=false )
	{
		...
		for ( $i=0 ; $i<count($this->_field) ; $i++ ) {
			...
			if ( $addAlias && $field !== '*' && strpos($field, '(') === false ) {
				...
				$a[] = str_replace('/','',$this->_protect_identifiers( $field ));
			}
			else {
	...
	protected function _build_order_field()
	{
	...
	protected function _build_inner()
	{
    ...
    /**
     * @param string $identifier_limiter
     */
    public function setIdentifierLimiter(array $identifier_limiter)
    {
        $this->_identifier_limiter = $identifier_limiter;
    }
	/**
	 * Protect field names
	...
	protected function _where ( $where, $value=null, $type='AND ', $op="=", $bind=true )
	{
		...
		foreach ($where as $key => $value) {
			...
			if ( $value === null ) {
				// Null query
                $opera = $this->_protect_identifiers($key) .( $op === '=' ?' IS NULL' : ' IS NOT NULL');
                $opera = $this->_protect_identifiers($key) .( $op === '' ? '' : '');
