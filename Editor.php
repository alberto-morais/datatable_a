...
//				throw new \Exception("Primary key element is not available in data set.", 1);
    ...
    public function removeQuotes( $query )
    {
        $this->_removeQuotes =  ['',''];
        $query->setIdentifierLimiter( $this->_removeQuotes );
    }

    public function inner ( $table = '', $condition = '', $type = 'INNER',$protectIdentifiers = true )
    {
        $this->_inner .= " $type JOIN $table on " .$condition.' ' ;
        $this->_auxInner[] = ['table' => $table, 'condition'=>$condition, 'type' => $type , 'protectIdentifiers' => $protectIdentifiers] ;
        return $this;
    }
    ...
    public function groupSet ( $_=null )
    {
        return $this->_getSet( $this->_groupSet, $_ );
    }


    public function orderFieldSet ( $_=null )
    {
        return $this->_getSet( $this->_orderFieldSet, $_ );
    }

    public function innerSet ( $_=null )
    {
        return $this->_getSet( $this->_inner, $_ );
    }
...
//		$this->_perform_inner_join( $query );
        ...
        $query->group( $this->_group );

        $query->orderField( $this->_orderField );
...
//        dd($this);
        ...
        $this->_perform_inner_join( $ssp_full_count );
    ...
    private function _ssp_filter ( $query, $http )
    {
        ...
        for ( $i=0, $ien=count($http['columns']) ; $i<$ien ; $i++ ) {
            ...
            $date = explode(' ',str_replace('/', '-',$search));
            if( isset($date[2])) {
                $date[0] = "'". date('Y-m-d', strtotime($date[0])) ."'";
                $date[2] = "'". date('Y-m-d', strtotime($date[2])) ."'";
                if ($column['search']['regex'] == 'BETWEEN Date'){
                    $column['search']['regex'] = 'BETWEEN ';
                    $query->where( "DATE({$column['data']}) ". ' ' .$column['search']['regex']." {$date[0]} {$date[1]} {$date[2]}");
                }else{
                    $query->where( $column['data'] . ' ' .$column['search']['regex']." {$date[0]} {$date[1]} {$date[2]}");
                }
            }else{
     ...
     *  @private
     */
    private function _perform_inner_join ( $query )
    {
        if(!empty($this->_auxInner)){
            foreach ($this->_auxInner as $value)
                $query->join( $value['table'], $value['condition'], $value['type'], $value['protectIdentifiers'] );
        }
    }
     ...
     *  @private
     */
    private function _get_group ( $query )
    {
        $query->group($this->_group);
    }
     ...
     *  @private
     */
    private function _get_orderField ( $query )
    {
        $query->orderField($this->_orderField);
    }
